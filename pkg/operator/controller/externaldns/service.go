...
// It creates one port per zone container, matching the ExternalDNS container metrics ports directly.
func desiredMetricsService(namespace string, externalDNS *operatorv1beta1.ExternalDNS) *corev1.Service {
	...
	for i := 0; i < numZones; i++ {
		portName := metricsPortNameForSeq(i)
		ports[i] = corev1.ServicePort{
			Name:       portName,
			Port:       int32(defaultMetricsStartPort + i),
