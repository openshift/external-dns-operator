...
```bash
$ ZONE_NAME=$(az network private-dns zone list -g "${RESOURCE_GROUP}" -o tsv --query '[0].name')
```

7. Create an ExternalDNS CR as follows:
```bash
$ cat <<EOF | oc create -f -
...
For private dns zones, create an ExternalDNS CR as follows:
```bash
$ cat <<EOF | oc create -f -
apiVersion: externaldns.olm.openshift.io/v1beta1
kind: ExternalDNS
