...
func EnsureOperandResources(kubeclient client.Client) error {
	if os.Getenv(e2eSeparateOperandNsEnvVar) != "true" {
		return nil
	}

	if err := ensureOperandNamespace(kubeclient); err != nil {
		return fmt.Errorf("Failed to create %s namespace: %v\n", operandNamespace, err)
	}

	if err := ensureOperandRole(kubeclient); err != nil {
		return fmt.Errorf("Failed to create role external-dns-operator in ns %s: %v\n", operandNamespace, err)
	}

	if err := ensureOperandRoleBinding(kubeclient); err != nil {
...
func ensureOperandNamespace(kubeClient client.Client) error {
	return kubeClient.Create(context.TODO(), &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: operandNamespace}})
}

func ensureOperandRole(kubeClient client.Client) error {
...
func ensureOperandRoleBinding(kubeClient client.Client) error {
...
func InitKubeClient() (error, client.Client, *kubernetes.Clientset) {
	kubeConfig, err := config.GetConfig()
	if err != nil {
		return fmt.Errorf("failed to get kube config: %w", err), nil, nil
	}

	kubeClient, err := client.New(kubeConfig, client.Options{})
	if err != nil {
		return fmt.Errorf("failed to create kube client: %w", err), nil, nil
	}
	...
	return nil, kubeClient, kubeClientSet
}
...
func GetPlatformType(isOpenShift bool, kubeClient client.Client) (error, string) {
	...
	if isOpenShift {
		if dnsProvider := os.Getenv(dnsProviderEnvVar); dnsProvider != "" {
			...
			return nil, string(infraConfig.Status.PlatformStatus.Type)
		}
	} else {
		platformType = MustGetEnv(dnsProviderEnvVar)
	}
	return nil, platformType
}
