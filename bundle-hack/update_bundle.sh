...
sed -i -e "s|quay.io/openshift/origin-external-dns-operator:latest|${OPERATOR_PULLSPEC}|g" \
       -e "s|k8s.gcr.io/external-dns/external-dns:.*$|${OPERAND_PULLSPEC}|g" \
       -e "s|quay.io/openshift/origin-external-dns:.*$|${OPERAND_PULLSPEC}|g" \
       -e "s|quay.io/openshift/origin-external-dns@.*$|${OPERAND_PULLSPEC}|g" \
       -e "s|quay.io/external-dns-operator/external-dns:.*$|${OPERAND_PULLSPEC}|g" \
       -e "s|quay.io/external-dns-operator/external-dns@.*$|${OPERAND_PULLSPEC}|g" \
       -e "s|quay.io/openshift/origin-kube-rbac-proxy:latest|registry.redhat.io/openshift4/ose-kube-rbac-proxy:v4.13|g" "${CSV_FILE}"
...
annotation_image_name = f'external-dns-operator-rhel9-operator-{operator_sha}-annotation'
